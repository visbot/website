...
import { meta as store } from '$stores/meta';
  ...
  function artistHandler(e = null) {
    ...
    if (value?.length) {
      store.set(
        $store.filter(item => item.artists.find(item => item.toLowerCase().startsWith(value)))
      );
    } else {
      store.set(metaData);
    }
  ...
  function typeHandler(e = null) {
    ...
    if (value?.length) {
      store.set(
        $store.filter(item => item.type === value)
      );
    } else {
      store.set(metaData);
    }
  ...
  function seriesHandler(e = null) {
    ...
    if (value?.length) {
      store.set(
        $store.filter(item => item.id.toLowerCase().startsWith(`v${value}`))
      );
    } else {
      store.set(metaData);
    }
...
<div class="hidden justify-start sm:flex flex-wrap sm:gap-x-4">
  <input class="grow bg-white sm:border-2 border-black dark:bg-neutral-900 dark:text-white p-4 my-0 sm:mb-4" type="text" on:keyup={searchHandler} bind:value={searchValue} />

  <select class="bg-white grow sm:border-2 border-black dark:bg-neutral-900 dark:text-white p-4 my-0 sm:mb-4" on:change={artistHandler} bind:value={selectedArtist}>
